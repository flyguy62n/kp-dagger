# KP Dagger - {Service/Component Name} Architecture

## Overview

Device parsing is a core component of the Dagger application.  This interface is responsible reading raw configuration files and transforming these configurations into a common data structure used throughout Dagger

Raw configurations are produced using native commands, usually through a logged SSH session:

- Cisco IOS and ASA: `show running-config`
- Fortigate: `show full-configuration`
- Sonicwall: `*.exp` configuration export
- And similar for other device types

## Architectural Principles

### Design Goals
- **Common Interface**: A common interface is used so that higher-level application logic remains unaware of configuration file syntax details
- **Factory pattern**: The factory pattern will be used to implement device-specific parser logic
...
{Concluding statement about the architecture and its benefits to the KP Dagger ecosystem.}
